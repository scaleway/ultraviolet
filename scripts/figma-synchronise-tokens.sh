...
function generateTokens {
  ...
  GENERATED_TOKENS_COLOR=$(echo "${JSON} $( cat "${TOKENS_EXTENSION}")" | jq -s '.[0] * .[1]' | jq --sort-keys --arg theme "${THEME}" --arg global "${GLOBAL}" --argjson shades "${PARSED_SHADES}" '.[$global] | with_entries(select(.value | has("backgroundStrong"))) | reduce (. | to_entries | .[]) as $sentiment ({}; . + {"\($sentiment.key)": (reduce($sentiment.value | to_entries | .[]) as $token ({}; . + { "\($token.key)": ($token.value.value | gsub("[$]"; ".") | split(".") as $number | $shades[$number[2]][$number[3]]) })) }) | {"colors": .}')
}
