...
Scaleway UI provide you a set of colors trough theme, those colors are defined by tokens following asana token pattern.
...
In this example there are different colors, but they all are having the same sentiment: `info`.
...
<ThemeProvider theme={theme}>
  <Global styles={[globalStyles]}/>
  <Alert variant="outlined" type="info" mt={2}>This is a simple alert with info sentiment and weak prominence</Alert>
</ThemeProvider>
...
It results in a different sets of colors and style for expressing a different context on where the alert is displayed.
...
<ThemeProvider theme={theme}>
  <Global styles={[globalStyles]}/>
  {Object.keys(localContrasts).map(
    sentiment => (
      <>
        <Typography variant="title" style={{textTransform: 'capitalize'}}>
          {sentiment}
        </Typography>
        <Row display="inline-flex" flexDirection="column" mr={2}>
          {Object.keys(theme.colors[sentiment]).filter(context => context.includes('background'))?.map(context => (
            <Box
              key={context}
              width={275}
              p={1}
              ...
              alignItems="center"
            >
              <Typography color={readableColor(theme.colors[sentiment][context])}>
                {context}
              </Typography>
              <Typography variant="tiny" color={readableColor(theme.colors[sentiment][context])}>
        ...
        <Row display="inline-flex" flexDirection="column" mr={2}>
          {Object.keys(theme.colors[sentiment]).filter(context => context.includes('text'))?.map(context => (
            <Box
              key={context}
              width={275}
              p={1}
        ...
        <Row display="inline-flex" flexDirection="column">
          {Object.keys(theme.colors[sentiment]).filter(context => context.includes('border'))?.map(context => (
            <Box
              key={context}
              width={275}
              p={1}
...
Keep in mind that it is an extension of existing theme, meaning that it will override the theme your values.
