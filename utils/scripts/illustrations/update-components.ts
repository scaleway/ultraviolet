...
const WIRE_ILLUSTRATIONS_DIR = 'packages/illustrations/src/components/WireIllustration/__generated__/'

const DYNAMIC_ILLUSTRATIONS_DIR = 'packages/illustrations/src/components/DynamicIllustration/__generated__'

const WARNING = `/**
      * This file is automatically generated from /utils/scripts/illustrations/update-components.ts
      * PLEASE DO NOT EDIT HERE
      */`

export const updateWireIllustrations = (illustrations: { dir: string; category: string }[], baseUrl: string) => {
  ...
  fs.writeFileSync(
    ...
    ${illustrations
      .map(element => {
        ...
        const categoryNotCapitalized = categorySnake.replaceAll(/-./gu, x => x[1].toUpperCase())
...
const filterDynamicIllustrations = (dynamicIllustrations: { dir: string; category: string }[]) => {
  ...
  return finalIllustrations.map(base => {
    ...
    const dir = dynamicIllustrations.find(i => i.category.startsWith(base))?.dir ?? ''
...
export const updateDynamicIllustrations = (
  ...
  const filteredDynamicIllustrations = filterDynamicIllustrations(dynamicIllustrations)
  ...
  fs.writeFileSync(
    ...
    export const ILLUSTRATIONS = {
      light: {
       ${filteredDynamicIllustrations
         .map(illustration => `${illustration.category}: ${illustration.category}Light,`)
         .join('\n')}
      } satisfies IllustrationsKeys,

      dark: {
      ${filteredDynamicIllustrations
        .map(illustration => `${illustration.category}: ${illustration.category}Dark,`)
        .join('\n')}
      } satisfies IllustrationsKeys,
      }
`,
