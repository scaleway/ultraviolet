...
- `disabled` (boolean): whether the option is disabled. _This prop is optional_.
- `description` (ReactNode): more information on the option. The description can be placed at the right or beneath the option label in the dropdown with the prop `direction`, which is **global**. When description is a string, it is also used when searching an option. _This prop is optional_.
...
- `tooltip` (string): If defined, a tooltip with the string defined will be shown when hovering the option in the dropdown. _This prop is optional_.
