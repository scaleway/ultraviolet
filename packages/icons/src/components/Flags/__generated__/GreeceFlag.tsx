'use client'

/**
 * Provide the icon component for the icon name.
 * This file is automatically generated from /utils/scripts/generate-icons-file.tsx.
 * PLEASE DO NOT EDIT HERE
 */
import { Icon } from '../Icon'

import type { IconProps } from '../Icon'

export const GreeceFlag = ({ ...props }: Omit<IconProps, 'children' | 'title'>) => (
  <Icon {...props} title="GreeceFlag">
    {props.disabled ? (
      <>
        <g>
          <path
            d="M12 24C18.6274 24 24 18.6274 24 12C24 5.37258 18.6274 0 12 0C5.37258 0 0 5.37258 0 12C0 18.6274 5.37258 24 12 24Z"
            fill="#F0F0F0"
          />
          <path d="M12 8.86947H23.5866C23.2857 7.75319 22.8284 6.70127 22.2387 5.73901H12V8.86947Z" fill="#7B7B7B" />
          <path
            d="M4.52991 21.3913H19.4696C20.5647 20.5191 21.5039 19.4596 22.2384 18.2609H1.76105C2.49563 19.4596 3.43481 20.5191 4.52991 21.3913V21.3913Z"
            fill="#7B7B7B"
          />
          <path d="M4.17366 2.90344C3.23025 3.71583 2.41467 4.6725 1.76105 5.73919H4.17366V2.90344Z" fill="#7B7B7B" />
          <path
            d="M12 12C12 10.7777 12 9.7547 12 8.86951H7.30434V12H4.17389V8.86951H0.413391C0.144375 9.86767 0 10.9168 0 12C0 13.0831 0.144375 14.1323 0.413391 15.1304H23.5867C23.8556 14.1323 24 13.0831 24 12H12Z"
            fill="#7B7B7B"
          />
          <path
            d="M11.9999 0C10.3335 0 8.74654 0.339984 7.3042 0.953812V5.73914H11.9999C11.9999 4.64639 11.9999 3.717 11.9999 2.60869H19.4697C17.4198 0.976125 14.824 0 11.9999 0V0Z"
            fill="#7B7B7B"
          />
        </g>
        <defs>
          <clipPath id="clip0_3072_434">
            <rect fill="white" />
          </clipPath>
        </defs>
      </>
    ) : (
      <>
        <g>
          <path
            d="M12 24C18.6274 24 24 18.6274 24 12C24 5.37258 18.6274 0 12 0C5.37258 0 0 5.37258 0 12C0 18.6274 5.37258 24 12 24Z"
            fill="#F0F0F0"
          />
          <path d="M12 8.86947H23.5866C23.2857 7.75319 22.8284 6.70127 22.2387 5.73901H12V8.86947Z" fill="#338AF3" />
          <path
            d="M4.52991 21.3913H19.4696C20.5647 20.5191 21.5039 19.4596 22.2384 18.2609H1.76105C2.49563 19.4596 3.43481 20.5191 4.52991 21.3913V21.3913Z"
            fill="#338AF3"
          />
          <path d="M4.17366 2.90344C3.23025 3.71583 2.41467 4.6725 1.76105 5.73919H4.17366V2.90344Z" fill="#338AF3" />
          <path
            d="M12 12C12 10.7777 12 9.7547 12 8.86951H7.30434V12H4.17389V8.86951H0.413391C0.144375 9.86767 0 10.9168 0 12C0 13.0831 0.144375 14.1323 0.413391 15.1304H23.5867C23.8556 14.1323 24 13.0831 24 12H12Z"
            fill="#338AF3"
          />
          <path
            d="M11.9999 0C10.3335 0 8.74654 0.339984 7.3042 0.953812V5.73914H11.9999C11.9999 4.64639 11.9999 3.717 11.9999 2.60869H19.4697C17.4198 0.976125 14.824 0 11.9999 0V0Z"
            fill="#338AF3"
          />
        </g>
        <defs>
          <clipPath id="clip0_3072_209">
            <rect fill="white" />
          </clipPath>
        </defs>
      </>
    )}
  </Icon>
)
