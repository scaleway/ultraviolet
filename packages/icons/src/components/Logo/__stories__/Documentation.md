...
1. Add the `.svg` file into `packages/icons/src/components/Logo/assets/light`.
   - The light version is **mandatory** (used to generate the component);
   - Optionally, add a dark version to `packages/icons/src/components/Logo/assets/dark` for better visibility in dark mode. The file name must match the light version exactly.
