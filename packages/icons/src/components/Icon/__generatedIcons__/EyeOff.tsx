/**
 * Provide the icon component for the icon name.
 * This file is automatically generated from /scripts/generateIconFiles.ts.
 * PLEASE DO NOT EDIT HERE
 */
import { Icon } from '../Icon'
import type { IconProps } from '../Icon'

export const EyeOff = ({ ...props }: Omit<IconProps, 'children'>) => (
  // eslint-disable-next-line react/jsx-props-no-spreading
  <Icon {...props}>
    <path
      fillRule="evenodd"
      d="M3.598 2.21a.715.715 0 0 0-1.011 1.01l13.816 13.816a.715.715 0 1 0 1.011-1.01l-1.663-1.663a9.56 9.56 0 0 0 3.144-4.172c.14-.364.14-.767 0-1.13A9.532 9.532 0 0 0 5.48 4.09zm4.26 4.26 1.04 1.04a2.382 2.382 0 0 1 3.214 3.214l1.042 1.041A3.811 3.811 0 0 0 7.859 6.47"
      clipRule="evenodd"
    />
    <path d="m10.714 13.368 2.403 2.403a9.5 9.5 0 0 1-3.116.522 9.53 9.53 0 0 1-8.896-6.108 1.57 1.57 0 0 1 0-1.13A9.5 9.5 0 0 1 3.176 5.83l3.08 3.08q-.066.348-.067.713a3.81 3.81 0 0 0 4.525 3.745" />
  </Icon>
)
