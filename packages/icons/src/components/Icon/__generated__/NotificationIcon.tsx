'use client'

/**
 * Provide the icon component for the icon name.
 * This file is automatically generated from /utils/scripts/generate-icons-file.tsx.
 * PLEASE DO NOT EDIT HERE
 */
import { Icon } from '../Icon'
import type { IconProps } from '../Icon'

export const NotificationIcon = ({ ...props }: Omit<IconProps, 'children' | 'title'>) => (
  <Icon {...props} title="NotificationIcon">
    {typeof props.size === 'string' && ['medium', 'large', 'xlarge', 'xxlarge'].includes(props.size) ? (
      <path
        d="M10 2a6 6 0 0 0-6 6 11.44 11.44 0 0 1-1.258 5.234.75.75 0 0 0 .515 1.076q1.603.333 3.257.508a3.5 3.5 0 0 0 6.972 0 33 33 0 0 0 3.256-.508.75.75 0 0 0 .515-1.076A11.45 11.45 0 0 1 16 8a6 6 0 0 0-6-6M8.049 14.943a34 34 0 0 0 3.902 0 2 2 0 0 1-3.902 0"
        clipRule="evenodd"
      />
    ) : (
      <path
        fillRule="evenodd"
        d="M8.002 1a5.25 5.25 0 0 0-5.25 5.25c0 1.65-.397 3.207-1.1 4.58a.656.656 0 0 0 .45.942q1.403.291 2.85.444a3.063 3.063 0 0 0 6.1 0 29 29 0 0 0 2.85-.444.656.656 0 0 0 .45-.942 10 10 0 0 1-1.1-4.58c0-2.9-2.35-5.25-5.25-5.25M6.296 12.325a29 29 0 0 0 3.413 0 1.75 1.75 0 0 1-3.413 0"
        clipRule="evenodd"
      />
    )}
  </Icon>
)

NotificationIcon.displayName = 'NotificationIcon'
