/**
 * Provide the icon component for the icon name.
 * This file is automatically generated from /scripts/generateIconFiles.ts.
 * PLEASE DO NOT EDIT HERE
 */
import { Icon } from '../Icon'
import type { IconProps } from '../Icon'

export const DocumentDb = ({ ...props }: Omit<IconProps, 'children'>) => (
  // eslint-disable-next-line react/jsx-props-no-spreading
  <Icon {...props}>
    <g className="Managed-Database-Document">
      <g className=".Square">
        <path
          fill="#F1EEFC"
          d="M0 16C0 7.163 7.163 0 16 0h32c8.837 0 16 7.163 16 16v32c0 8.837-7.163 16-16 16H16C7.163 64 0 56.837 0 48z"
          className="fillWeak"
        />
      </g>
      <g className="icon">
        <path
          fill="#A060F6"
          d="M16.852 24.977a1 1 0 0 0-1.72 1.02L16 25.5c-.868.496-.867.497-.867.498l.002.003.002.005.007.01.015.025a2.673 2.673 0 0 0 .176.242c.114.14.277.316.507.517.459.401 1.173.892 2.257 1.366C20.264 29.113 23.911 30 30 30a1 1 0 1 0 0-2c-5.91 0-9.263-.863-11.099-1.666-.915-.4-1.451-.785-1.742-1.04a2.366 2.366 0 0 1-.307-.317m0 9.5a1 1 0 0 0-1.72 1.02L16 35c-.868.496-.867.497-.867.498l.001.002.002.004.006.01.012.02a1.998 1.998 0 0 0 .136.193c.086.11.207.25.373.41.334.32.844.718 1.605 1.127 1.52.816 4.033 1.67 8.142 2.043a1 1 0 1 0 .18-1.992c-3.915-.355-6.151-1.156-7.376-1.813-.61-.328-.97-.621-1.165-.808a1.88 1.88 0 0 1-.197-.217"
          className="fillStrong"
          clipRule="evenodd"
          fillRule="evenodd"
        />
        <path
          fill="#521094"
          d="m30 12-.788.006C21.248 12.122 15 14.002 15 17v25.455l.006.163C15.274 45.906 21.886 48 30 48c.349 0 .694-.004 1.037-.011A12.99 12.99 0 0 0 42 54c7.18 0 13-5.82 13-13 0-6.147-4.266-11.298-10-12.652V17c0-3.096-6.664-5-15-5m13 16.038V19.65c-2.46 1.424-6.964 2.267-12.211 2.343h-.008a49.93 49.93 0 0 1-.777.006H30c-5.583 0-10.416-.854-13-2.349v22.763l.002.082c.137 1.669 5.929 3.503 12.995 3.504A12.96 12.96 0 0 1 29 41c0-7.18 5.82-13 13-13 .337 0 .67.013 1 .038m0-11.053c-.026-.448-1.196-1.23-3.375-1.852C37.113 14.415 33.676 14 30 14s-7.112.415-9.625 1.133C18.173 15.763 17 16.555 17 17l.006.055c.086.435 1.16 1.146 3.114 1.737l.252.074h.003C22.889 19.586 26.325 20 30 20c2.505 0 4.898-.193 6.964-.543.587-.1 1.148-.211 1.677-.335.342-.08.67-.165.984-.255l.046-.013c.325-.094.627-.191.906-.29 1.532-.545 2.364-1.145 2.42-1.524zM31 41c0-6.075 4.925-11 11-11s11 4.925 11 11-4.925 11-11 11-11-4.925-11-11"
          className="fill"
          clipRule="evenodd"
          fillRule="evenodd"
        />
        <path
          fill="#A060F6"
          d="M36.475 40.535a1 1 0 0 0 0 1.377l.964 1.016v2.011a2.508 2.508 0 0 0 2.508 2.508 1 1 0 0 0 0-2 .508.508 0 0 1-.508-.508v-2.41a1 1 0 0 0-.275-.688l-.586-.617.586-.618a1 1 0 0 0 .275-.688v-2.41c0-.28.227-.508.508-.508a1 1 0 0 0 0-2 2.508 2.508 0 0 0-2.508 2.508v2.01zm10.487-1.016.964 1.016a1 1 0 0 1 0 1.377l-.964 1.016v2.011a2.508 2.508 0 0 1-2.508 2.508 1 1 0 0 1 0-2c.28 0 .508-.227.508-.508v-2.41a1 1 0 0 1 .274-.688l.586-.617-.586-.618a1 1 0 0 1-.274-.688v-2.41a.508.508 0 0 0-.508-.508 1 1 0 1 1 0-2 2.508 2.508 0 0 1 2.508 2.508v2.01Z"
          className="fillStrong"
        />
      </g>
    </g>
  </Icon>
)
