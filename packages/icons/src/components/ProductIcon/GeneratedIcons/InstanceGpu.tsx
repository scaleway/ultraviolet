/**
 * Provide the icon component for the icon name.
 * This file is automatically generated from /scripts/generateIconFiles.mjs.
 * PLEASE DO NOT EDIT HERE
 */
import { Icon } from '../Icon'
import type { IconProps } from '../Icon'

export const InstanceGpu = ({ ...props }: Omit<IconProps, 'children'>) => (
  // eslint-disable-next-line react/jsx-props-no-spreading
  <Icon {...props}>
    <g className="Instance-GPU">
      <g className=".Square">
        <path
          fill="#EEF"
          d="M0 16C0 7.163 7.163 0 16 0h32c8.837 0 16 7.163 16 16v32c0 8.837-7.163 16-16 16H16C7.163 64 0 56.837 0 48z"
          className="fillWeak"
        />
      </g>
      <path
        fill="#4F0599"
        d="M25.493 12.883A1 1 0 0 0 23.5 13v5H23a5 5 0 0 0-5 5v.5h-5l-.117.007A1 1 0 0 0 13 25.5h5v3h-5l-.117.007A1 1 0 0 0 13 30.5h5v3h-5l-.117.007A1 1 0 0 0 13 35.5h5v3h-5l-.117.007A1 1 0 0 0 13 40.5h5v.5a5 5 0 0 0 5 5h.5v4.828l.007.117a1 1 0 0 0 1.993-.117V46h3v4.828l.007.117a1 1 0 0 0 1.993-.117V46h3v4.828l.007.117a1 1 0 0 0 1.993-.117V46h3v4.828l.007.117a1 1 0 0 0 1.993-.117V46h.5a5 5 0 0 0 5-5v-.5h4.828l.117-.007a1 1 0 0 0-.117-1.993H46v-3h4.828l.117-.007a1 1 0 0 0-.117-1.993H46v-3h4.828l.117-.007a1 1 0 0 0-.117-1.993H46v-3h4.828l.117-.007a1 1 0 0 0-.117-1.993H46V23a5 5 0 0 0-5-5h-.5v-5l-.007-.117A1 1 0 0 0 38.5 13v5h-3v-5l-.007-.117A1 1 0 0 0 33.5 13v5h-3v-5l-.007-.117A1 1 0 0 0 28.5 13v5h-3v-5zM44 24.5V23a3 3 0 0 0-2.824-2.995L41 20H23a3 3 0 0 0-2.995 2.824L20 23v18a3 3 0 0 0 2.824 2.995L23 44h18a3 3 0 0 0 2.995-2.824L44 41z"
        className="fill"
        clipRule="evenodd"
        fillRule="evenodd"
      />
      <path
        fill="#A365F6"
        d="M31.074 30.451c-1.62-.861-2.306-2.836-1.518-4.494.166-.349.312-.648.413-.838.1-.187.263-.47.456-.798.938-1.587 2.967-2.125 4.595-1.26.637.34.817 1.17.378 1.742l-1.562 2.035a1.803 1.803 0 0 0-.28.526l-.809 2.42a1.18 1.18 0 0 1-1.673.667m-4.997 4.114c1.658.787 3.633.1 4.494-1.52a1.18 1.18 0 0 0-.668-1.672l-2.419-.809a1.802 1.802 0 0 1-.526-.28l-2.034-1.562a1.168 1.168 0 0 0-1.742.378c-.866 1.628-.328 3.657 1.259 4.595.327.194.611.357.799.456.19.101.488.248.838.413Zm8.608 3.478c.787-1.658.101-3.633-1.519-4.494a1.18 1.18 0 0 0-1.673.668l-.809 2.419c-.063.19-.158.367-.28.526l-1.561 2.035c-.44.572-.26 1.403.378 1.742 1.627.865 3.657.327 4.594-1.26.194-.327.357-.611.457-.798.1-.19.247-.49.413-.838m3.478-8.607c-1.658-.788-3.632-.102-4.494 1.518a1.18 1.18 0 0 0 .668 1.673l2.419.809c.19.063.367.158.526.28l2.035 1.562c.572.44 1.403.26 1.742-.378.865-1.628.327-3.657-1.26-4.595a23.168 23.168 0 0 0-.798-.456c-.19-.101-.489-.247-.838-.413"
        className="fillStrong"
        clipRule="evenodd"
        fillRule="evenodd"
      />
    </g>
  </Icon>
)
