...
1. ✅ **Perceivable**: the collection of tags is conveyed as a list, so assistive technology knows that the tags form a cohesive set (WCAG 1.3.1).
...
- ✅ [WCAG 1.3.1 - Info and Relationships (Level A)](https://www.w3.org/WAI/WCAG22/Understanding/info-and-relationships.html) - The tags are rendered inside a list so screen readers can convey the grouping and count.
...
- ✅ The visible tags and the hidden tags inside the popover are plain `<div>`s, so the collection is not conveyed to assistive technology (WCAG 1.3.1).
  - The component now renders a `<ul>` with each tag in an `<li>` for the visible container and for the popover content. The `+N` counter remains a sibling of the list, not a list item.
