...
describe('snippet', () => {
  ...
  it('renders correctly with hideText', () => {
    shouldMatchSnapshot(<Snippet hideText="Test">{TEST_VALUE_MULTILINE}</Snippet>)
  })

  it('renders correctly with showText', () => {
    const { asFragment } = renderWithTheme(<Snippet showText="Test">{TEST_VALUE_MULTILINE}</Snippet>)
    expect(asFragment()).toMatchSnapshot()
  })

  it('renders correctly with initiallyExpanded', () => {
    const { asFragment } = renderWithTheme(<Snippet initiallyExpanded>{TEST_VALUE_MULTILINE}</Snippet>)
    expect(asFragment()).toMatchSnapshot()
  })
  ...
  it('should click on extend button to display full content on ', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /Show.*/u }))

    expect(asFragment()).toMatchSnapshot()
  })

  it('renders correctly with custom number of rows', () => {
    const { asFragment } = renderWithTheme(<Snippet rows={12}>{TEST_VALUE_MULTILINE}</Snippet>)
    expect(asFragment()).toMatchSnapshot()
  })

  it('renders correctly with noExpandable', () => {
    const { asFragment } = renderWithTheme(<Snippet noExpandable>{TEST_VALUE_MULTILINE}</Snippet>)
    expect(asFragment()).toMatchSnapshot()
  })
