...
* `disabled` (boolean): whether the option is disabled. *This prop is required*.
* `description` (string): more information on the option. The description can be placed at the right or beneath the option label in the dropdown with the prop `direction`, which is **global**. *This prop is optional*.
...
* `searchText` (string): define here what should the user search to get this option. *This prop is optional*.
