...
describe('numberInputField', () => {
  ...
  it('should work fine with form setValues', async () => {
    ...
    // Use setValue for each field individually to ensure proper re-render
    // setValues has a known issue where it doesn't trigger useController re-renders
    act(() => {
      resultForm.current.setValue('test1', 11, {
        shouldValidate: true,
        shouldDirty: true,
        shouldTouch: true,
      })
      resultForm.current.setValue('test2', 21, {
        shouldValidate: true,
        shouldDirty: true,
        shouldTouch: true,
      })
    ...
    /**
     * All this part is waiting an issue on react hook form.
     */
